...
//			@PageableDefault(page=0,size=10) // 배열형식이라 0 으로 시작
//			@SortDefaults({
...
//		return (List<BoardDto>) map.get("list");
//	}
	...
	public String blist(
		...
		if(search == null) {
			...
			map = boardService.findContains(page,size,category,search);
		}
		model.addAttribute("map", map);
		return "blist";
	}
...
//			@RequestParam(name="size",defaultValue = "10") int size,
//			Model model) {
//		Page<BoardDto> pageList = boardService.findAll(page,size);
//		return pageList;
//	}
